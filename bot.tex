...
\parbox[4mm]{7mm }{\centering $R^2$ }    &
\parbox[7mm]{7mm }{\centering CVM 1se }  & 
\parbox[10mm]{7mm }{\centering $R^2$ 1se} & 
\parbox[7mm]{7mm }{\centering CVM min }  & 
\parbox[15mm]{7mm }{\centering $R^2$ min} 
        \\ \hline \hline
\scriptsize
...
\end{tabular}
\caption{ \label{fig:prediction} (b) Elastic net regression and comparison to
linear models based on a global deformation approach.(a) Permutation test for
the transport based regression fits.The y-Axis is scaled by the cross-validated
mean error, thus for Age the permuted results are on average 1.5 times above
the non-permuted fit (p--value 0), for CDR 1.35 times (p--value 0) and for Age
1.1 times (p-value 0.03) based on 1000 permutations.} 
\end{figure}
